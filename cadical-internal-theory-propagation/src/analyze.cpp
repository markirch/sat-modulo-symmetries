...
inline void Internal::bump_also_reason_literals (int lit, int limit) {
  ...
  if (!reason || reason == external_reason) return;
  for (const auto & other : *reason) {
...
inline int Internal::determine_actual_backtrack_level (int jump) {
  ...
  if (!opts.chrono && !external->propagator) {
    res = jump;
...
void Internal::analyze () {
  ...
  if (external->propagator && !external->propagator->is_lazy) explain_external_propagations ();
