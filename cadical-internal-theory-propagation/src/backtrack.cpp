...
void Internal::backtrack (int new_level) {
  ...
  if (external->propagator && !external->propagator->is_lazy && notified > assigned) {
    LOG("external propagator is notified about some unassignments (trail: %zd, notified: %zd).",trail.size(),notified);
    notified = assigned;
  }
