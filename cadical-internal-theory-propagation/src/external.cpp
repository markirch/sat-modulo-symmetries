...
void External::add_observed_var (int elit) {
  if (!propagator) {
    LOG ("No connected propagator that could watch the variable, watch is not set.");
    return;
  }
  ...
  int eidx = abs (elit);
  if (eidx <= max_var && (marked (witness, elit) || marked (witness, -elit))) {
    LOG("Error, simplified variable is not allowed to become observed.");
    assert(false);
    
    LOG ("marking tainted %d", elit);
    mark (tainted, elit);
    mark (tainted, -elit);
    // restore_clauses must be called before
  }
 
  if (eidx >= (int64_t) is_observed.size ())
    is_observed.resize (1 + (size_t) eidx, false);
    
  if (is_observed[eidx]) return;
...
void External::remove_observed_var (int elit) {
  ...
  if (is_observed[eidx]) {
    // Follow opposite order of add_observed_var, first remove internal is_observed
...
void External::reset_observed_vars () {
  // Shouldn't be called again after propagator is disconnected
  assert (propagator);
  reset_extended ();
...
bool External::observed (int elit) {
  ...
  int eidx = abs (elit);
  if (eidx > max_var) return false;
  if (eidx >= (int) is_observed.size ()) return false;
