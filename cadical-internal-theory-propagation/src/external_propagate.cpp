...
void Internal::remove_observed_var (int ilit) {
  ...
  // level == 0 ensures that because there is no IG.
  
  // const int tmp = val (ilit);
  // if (!fixed(ilit) && tmp ) {
  //   Var & v = var (idx);
  //   if (v.reason == external_reason) {
  //     int target_lit =  tmp < 0 ? -idx : idx;
      
  //     v.reason = learn_external_reason_clause (target_lit);
  //     if (!v.reason) {
  //       v.level = 0;
  //       learn_unit_clause (target_lit);
  //     }
  //   }
  // }
  ...
  if (!conflict && external->propagator && !external->propagator->is_lazy) {
    ...
    if (!unsat && !conflict) {
      ...
      while (has_external_clause) {
        int level_before = level;
        // If the external lemma had a new variable, the solver had to backtrack to
        // level 0 in order to initialize it.
...
bool Internal::external_check_solution () {
  ...
  while (trail_changed || added_new_clauses) {
    ...
    while (has_external_clause) {
      ...
      //   for incremental use cases). TODO: work it around without backtrack.
...
void Internal::notify_assignments () {
  ...
  while (notified < end_of_trail) {
    ...
    int elit = externalize(ilit); //TODO: double-check the consequences of it 
    assert (elit);
...
void Internal::check_watched_literals () {
      // new val does not support that anymore:
