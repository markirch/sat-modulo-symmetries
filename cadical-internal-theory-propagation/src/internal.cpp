...
:
  ...
  conflict (0),
  ignore (0),
  external_reason (&external_reason_clause),
  propagated (0),
  propagated2 (0),
  propergated (0),
  best_assigned (0),
...
void Internal::enlarge (int new_max_var) {
  assert (!level || external->propagator);
...
void Internal::init_vars (int new_max_var) {
  ...
  if (level && !external->propagator) backtrack ();
...
int Internal::cdcl_loop_with_inprocessing () {
  ...
  while (!res) {
      ...
      (!external_propagate () || unsat) {    // propagate externally 
        if (unsat) continue;
        else analyze ();                     
    }           
...
int Internal::solve (bool preprocess_only) {
  ...
  if (!preprocess_only) {
    ...
    if (!res || external->propagator) res = cdcl_loop_with_inprocessing ();
  }
  reset_solving ();
  report_solving (res);
  STOP (solve);
