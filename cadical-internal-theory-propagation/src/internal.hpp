...
struct Internal {
  ...
  Clause * external_reason;     // used when cb_propagate () propagates
  ...
  size_t notified;              // next trail position to notify_assignment
  ...
  // Learning from external solver in 'external_propagate.cpp'
  //
  ...
  int ask_decision ();
  // Handling relevant (observed) variables of external propagator
  //
  void add_observed_var (int ilit);
  void remove_observed_var (int ilit);
  bool observed (int ilit) const;
  bool is_decision (int ilit);
  // Does not do anything, only assertions to check properties:
  //
  void check_watched_literals (); 
