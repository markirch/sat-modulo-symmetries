...
private:
  ...
  // Internal book keeping about where are we in already thought lemmas
  size_t query_loc = 0;
  size_t lemma_loc = 0;
...
public:
  MockPropagator (Solver *  solver)  { 
    std::cout<< "Mock propagator created." << std::endl;
    observed_trail.push_back(std::vector<int>());
    s = solver;
  }

  ~MockPropagator () {
    std::cout << "MockPropagator is freed up." << std::endl;
  }
...
void push_lemma_lit (int lit) {
  ...
  void report_internal_stats () {
    std::cout << "Number of observed variables: " 
      << observed_variables.size() << std::endl;
    std::cout << "Number of lemmas stored: "
      << nof_clauses << "-" << all_external_clauses.size()
      << std::endl;
    std::cout << "Lemma-passing-pattern: ";
    for (auto nof_lemma_to_teach : lemmas_per_queries) {
      std::cout << nof_lemma_to_teach << "-";
    }
    std::cout << std::endl;
  ...
  int add_new_observed_var () {
    for (std::vector<int>::iterator it = new_observed_variables.begin(); it != new_observed_variables.end();) {
      int lit = * it;
      if (s->is_witness(lit)) continue;
      new_observed_variables.erase (it);
      observed_variables.insert(lit);
      std::cout << "New observed variable is added: " << lit << std::endl;
  ...
  int remove_new_observed_var () {
    if (added_new_observed_variables.size()) {
      int lit = added_new_observed_variables.back();
      added_new_observed_variables.pop_back();
      assert (s->observed (lit));
      new_observed_variables.push_back(lit);
      observed_variables.erase(lit);
      std::cout << "Observed variable is removed: " << lit << std::endl;
...
bool cb_check_found_model (const std::vector<int> & model) {
  // 'all_external_clauses' contains also the propagating (but not necessarily learnt)
  // clauses. The final solution must satisfy only the initial input set of clauses.
  assert ( model.size() == observed_variables.size() );

#ifndef NDEBUG
  std::cout  << "model: ";
  for (const auto lit: model) std::cout << lit << " ";
  std::cout << std::endl;

  std::cout << "nof clauses: " << nof_clauses << std::endl;
  std::cout << "nof added clauses: " << nof_added_clauses << std::endl;
  assert (nof_added_clauses <= nof_clauses);
  
  for (size_t i = 0; i < nof_clauses; i++ ) {
    for (const auto lit : all_external_clauses[i]) std::cout << lit << " ";
    std::cout << std::endl;
  }
#endif
  ...
  for (size_t i = 0; i < nof_clauses; i++ ) {
    ...
    if (!satisfied) {
#ifndef NDEBUG
      std::cout << "unsatisfied external clause (pos " << i << "):";
      for (const auto lit : all_external_clauses[i]) std::cout << lit << " ";
      std::cout << std::endl;
#endif
...
bool cb_has_external_clause () {
  ...
  if (query_loc >= lemmas_per_queries.size()) return false;
  if (lemmas_per_queries[query_loc] > 0) {
    add_new_observed_var ();
    lemmas_per_queries[query_loc]--;
#ifndef NDEBUG
    std::cout << "MockPropagator found external clause to add." << std::endl;
#endif
    return true;
  } else {
    query_loc++;
#ifndef NDEBUG
    std::cout << "MockPropagator did not find any external clause to add." << std::endl;
#endif
    return false;
  }
}
...
int cb_add_external_clause_lit () {
  ...
  std::cout << lit << " ";
  if (!lit) { lemma_loc++; lemma_lit_loc = 0; nof_added_clauses++; std::cout << std::endl;}
  return lit;
}

int cb_decide () {
  if (observed_variables.empty() || observed_variables.size() <= 4) return 0;

  if (!(observed_variables.size() % 5) && new_observed_variables.size()) {
    int new_var = add_new_observed_var ();
    if (new_var) return -1*new_var; 
  }
  decision_loc++;
...
#ifndef NDEBUG
      std::cout << "MockPropagator enforces decision:" << -1*lit << std::endl;
#endif
      return -1*lit;
    } else return 0;
  }
...
int cb_propagate () {
  ...
  if (observed_trail.size() < 2) return 0;
  std::set<int> satisfied_literals;
  std::vector<int> satisfied_literals_ends;
  size_t lit_sum = 0;
  int last_lit = 0;
  std::cout << "current trail: ";
  for (auto level_lits : observed_trail) {
    for (auto lit : level_lits) {
      if (!s->observed(lit)) continue;
      satisfied_literals.insert(lit);
      std::cout << lit << " ";
      lit_sum += abs(lit);
      last_lit = lit;
      if (!satisfied_literals_ends.size()) satisfied_literals_ends.push_back(lit);
    }
  }
  std::cout << std::endl;
  if (last_lit) satisfied_literals_ends.push_back(last_lit);
  else return 0;
  ...
  if (!unassigned_var) return 0;

  bool propagated = false;
  if (lit_sum % 5 == 0 && satisfied_literals.size() > 1) {
    all_external_clauses.emplace_back(
      std::initializer_list<int>{
        unassigned_var, -1*satisfied_literals_ends.back(), -1*satisfied_literals_ends.front(), 0
      }
    );
    propagated = true;
    ...
    all_external_clauses.emplace_back(std::initializer_list<int>{unassigned_var, 0});
    propagated = true;
    ...
    // It is ok if l1 == l2
    all_external_clauses.emplace_back(std::initializer_list<int>{l1, l2, 0});
    reason_map[l1] = all_external_clauses.size()-1;
    prop_reason_loc[l1] = 0;
#ifndef NDEBUG
    std::cout << "MockPropagator propagates: " << -1*satisfied_literals_ends.back() << std::endl;
#endif    
    return l1;
  } 
...
#ifndef NDEBUG
  if (propagated) {
    std::cout << "MockPropagator propagates: " << unassigned_var << " (with reason: ";
    for (const auto lit: all_external_clauses.back()) std::cout << lit << " ";
    std::cout << ")" << std::endl;
  } else std::cout << "MockPropagator did not find propagating clause." << std::endl;
#endif
...
int cb_add_reason_clause_lit (int plit) {
  ...
  assert( prop_reason_loc[plit] < all_external_clauses[reason_map[plit]].size() );
  int lit = all_external_clauses[reason_map[plit]][prop_reason_loc[plit]++];
  if (!lit) {
    prop_reason_loc[plit] = 0; // in case it is needed again
#ifndef NDEBUG
    std::cout << "reason clause passed: " <<  std::endl;
    for (const auto l: all_external_clauses[reason_map[plit]]) std::cout << l << " ";
    std::cout << std::endl;
#endif
  } 
  return lit;
...
void notify_new_decision_level () { observed_trail.push_back(std::vector<int>()); }

void notify_backtrack (size_t new_level) {
  assert(observed_trail.size() == 1 || observed_trail.size() >= new_level + 1);
  while (observed_trail.size() > new_level + 1) {
    // std::cout << "undo level " << observed_trail.size()-1 << ": ";
    // for (auto lit : observed_trail.back()) std::cout << lit << " ";
    // std::cout << std::endl;
    observed_trail.pop_back();
  }
  //report_internal_stats();
}
...
//   INIT
//   (SET|ALWAYS)*
//   (   (ADD|ASSUME|ALWAYS|OBSERVE)*
//       [ (SOLVE|SIMPLIFY|LOOKAHEAD) (LEMMA|CONTINUE)* (VAL|FLIP|FAILED|ALWAYS)* ]
//   )*
//   [ RESET ]
//
...
struct Call {
  ...
  enum Type {
    ...
    BEFORE = ADD | CONSTRAIN | ASSUME | ALWAYS | DISCONNECT | CONNECT | OBSERVE,
...
struct ConnectCall : public Call {
  ...
  void execute (Solver * & s) { s->connect_external_propagator (new MockPropagator (s)); }
...
struct ObserveCall : public Call {
  ...
  void execute (Solver * & s) { 
    //if (arg) s->add_observed_var (arg);
    MockPropagator * mp = static_cast<MockPropagator *>(s->get_propagator());
    if (mp) {
...
#undef SIGNAL
  ...
  void execute () {
    ...
    for (size_t i = 0; i < calls.size (); i++) {
      ...
      if (c->type == Call::LEMMA) continue;
      if (c->type == Call::CONTINUE) continue;
      ...
      if (c->type == Call::SOLVE) {
        ...
        for (size_t j = i + 1; j < calls.size(); j++) {
          Call * next_c = calls[j];
          if (next_c->type == Call::LEMMA) next_c->execute(solver);
          else if (next_c->type == Call::CONTINUE) next_c->execute(solver);
          else break;
        }
      }
...
void Trace::generate_propagator (Random & random,
                             int minvars, int maxvars) {
  if (random.generate_double () < 0.15) return;                              
  
  assert (minvars <= maxvars);                              
  if (in_connection) push_back (new DisconnectCall ());
  ...
  for (int idx = minvars; idx <= maxvars; idx++) {
    if (random.generate_double () < 0.6) continue;
  ...
  for (int idx = maxvars+1; idx <= maxvars*1.5; idx++) {
    if (random.generate_double () < 0.75) continue;
    int lit = random.generate_bool () ? -idx : idx;
    push_back (new ObserveCall (lit));
    observed_vars.push_back (abs(lit));
  }
...
void Trace::generate_lemmas (Random & random) {
  if (!observed_vars.size()) return;
  int nof_user_propagation_phases = random.pick_int (3, 12);
  
  for (int p = 0; p < nof_user_propagation_phases; p++) {
    if (random.generate_double () < 0.05) {
      push_back (new ContinueCall ());
    } else {
      const int nof_lemmas = random.pick_int (4, 15);
      ...
      for (int i = 0; i < nof_lemmas; i++) {
        ...
        if (random.generate_double () < 0.005) {
          ...
          if (count > ovars) count = ovars;
          const int max_idx = ovars - 1;
          bool * picked = new bool [max_idx + 1];
          for (int i = 0; i <= max_idx; i++) picked[i] = false;
          for (int i = 0; i < count; i++) {
            int idx;
            do idx = random.pick_int (0, max_idx); while (picked[idx]);
            picked[idx] = 1;
            int lit = random.generate_bool () ? -observed_vars[idx] : observed_vars[idx];
          ...
          // int len = 0;        
          // while (len < count) {
          //   // Duplicated and tautological literals are allowed
          //   int idx = random.pick_int (0, ovars - 1);
          //   int lit = random.generate_bool () ? -observed_vars[idx] : observed_vars[idx];
...
void Trace::generate_process (Random & random) {
  ...
  if (fraction < 0.6)  {
    ...
    if (in_connection && observed_vars.size()) generate_lemmas (random);
  }
...
void Trace::generate (uint64_t i, uint64_t s) {
  ...
  for (int call = 0; call < calls; call++) {
    ...
    if (in_connection && random.generate_bool ()) {
      ...
      in_connection = false;
    } else generate_propagator (random, minvars, maxvars);
...
// '(BEFORE*| PROCESS DURING* |AFTER*)' or single non-configuration calls.
//
...
bool Trace::shrink_userphases (int expected) {
  ...
  for (; l < size () && !during_type (calls[l]->type); l++)
    ;
  for (; l < size (); l++) {
    if (!during_type (calls[l]->type)) continue;
    r = l;
    while (r < size () && calls[r]->type == Call::LEMMA) r++;
...
void Trace::shrink (int expected) {
  ...
  do {
    ...
    if (l != UPHASES && shrink_userphases (expected))s = true, l = UPHASES;
    if (l != LEMMAS && shrink_lemmas (expected))     s = true, l = LEMMAS;
...
void Reader::parse () {
  int ch, lit = 0, val = 0, state = 0, adding = 0, constraining = 0, lemma_adding = 0, solved = 0;
  ...
  while ((ch = next ()) != EOF) {
    ...
    if (!strcmp (keyword, "init")) {
      ...
      if (!first) error ("argument to 'observe' missing");
      if (!parse_int_str (first, lit))
        error ("invalid argument '%s' to 'observe'", first);
      if (second) error ("additional argument '%s' to 'observe'", second);
      c = new ObserveCall (lit);
    } else if (!strcmp (keyword, "lemma")) {
      if (!first) error ("argument to 'lemma' missing");
      if (!parse_int_str (first, lit))
        error ("invalid argument '%s' to 'lemma'", first);
      if (second) error ("additional argument '%s' to 'lemma'", second);
  ...
  if (enforce) {
    ...
    if (lemma_adding && c->type != Call::LEMMA && c->type != Call::RESET)
      error("'%s' after 'lemma %d' without 'lemma 0'", c->keyword(), lemma_adding);
    ...
    case Call::FAILED:
      ...
      assert(state == Call::SOLVE || state == Call::SIMPLIFY ||
             ...
             state == Call::OBSERVE || state == Call::LEMMA || state == Call::CONTINUE ||
             state == Call::AFTER);
