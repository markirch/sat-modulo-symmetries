...
inline int Internal::assignment_level (int lit, Clause * reason) {
  ...
  assert (opts.chrono || external->propagator );
  if (!reason || reason == external_reason) return level;
...
inline void Internal::search_assign (int lit, Clause * reason) {
  ...
  assert (!vals[idx]);
  assert (!flags (idx).eliminated () || reason == decision_reason || reason == external_reason);
  Var & v = var (idx);
  int lit_level;
  
  if (reason == external_reason && ((size_t) level <= assumptions.size () + (!!constraint.size ()))) {
