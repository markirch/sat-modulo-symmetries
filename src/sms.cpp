...
// Adaptive automorphism tracking: Directory and file management
void GraphSolver::createDirectory(const std::string& dirName) {
  try {
    std::filesystem::create_directories(dirName);
  } catch (const std::filesystem::filesystem_error& e) {
    printf("⚠️  Warning: Could not create directory '%s': %s\n", dirName.c_str(), e.what());
  }
}

void GraphSolver::removeDirectory(const std::string& dirName) {
  try {
    if (std::filesystem::exists(dirName)) {
      std::filesystem::remove_all(dirName);
    }
  } catch (const std::filesystem::filesystem_error& e) {
    printf("⚠️  Warning: Could not remove directory '%s': %s\n", dirName.c_str(), e.what());
  }
}

void GraphSolver::saveGraphToFile(const adjacency_matrix_t& matrix, const std::string& filename) {
  try {
    std::ofstream file(filename);
    if (!file.is_open()) {
      printf("⚠️  Warning: Could not open file '%s' for writing\n", filename.c_str());
      return;
    }
    ...
    file << "# Graph with " << maxAutomorphismsFound << " automorphisms - Graph #" << graphsWithMaxCount << "\n";
    file << "# Vertices: " << matrix.size() << "\n";
    file << "# Timestamp: " << std::put_time(std::localtime(&time_t), "%Y-%m-%d %H:%M:%S") << "\n";
    file << "# Generated by SMS adaptive automorphism tracking\n";
    
    // Use existing SMS graph printing logic for consistency
    file << "[";
    bool first = true;
    ...
    printf("⚠️  Warning: Error saving graph to '%s': %s\n", filename.c_str(), e.what());
  }
}